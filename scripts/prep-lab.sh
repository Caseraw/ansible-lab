...
printf '%*s\n' "${COLUMNS:-$(tput cols)}" '' | tr ' ' -
echo -e "${GREEN}Install additional system packages${NC}"
printf '%*s\n' "${COLUMNS:-$(tput cols)}" '' | tr ' ' -
sudo dnf install git tree lsof bash-completion -y &&\

printf '%*s\n' "${COLUMNS:-$(tput cols)}" '' | tr ' ' -
echo -e "${GREEN}Show python details${NC}"
printf '%*s\n' "${COLUMNS:-$(tput cols)}" '' | tr ' ' -
...
printf '%*s\n' "${COLUMNS:-$(tput cols)}" '' | tr ' ' -
echo -e "${GREEN}Install pip packages${NC}"
printf '%*s\n' "${COLUMNS:-$(tput cols)}" '' | tr ' ' -
python3 -m pip install --user --upgrade pip setuptools ansible

printf '%*s\n' "${COLUMNS:-$(tput cols)}" '' | tr ' ' -
echo -e "${GREEN}Show ansible details${NC}"
printf '%*s\n' "${COLUMNS:-$(tput cols)}" '' | tr ' ' -
ansible --version
